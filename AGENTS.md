...
├── yt-dlp/
│   ├── config/
│   │   └── yt-dlp.conf       # Default yt-dlp config (generated by ./scripts/setup/init)
...
./scripts/service/start                    # Uses USE_VPN from .env
./scripts/service/start_no_vpn            # Force no-VPN mode regardless of .env

# Stop services
./scripts/service/stop                     # Stops all profiles, cleans up Podman pods/networks

# Restart services
./scripts/service/restart                  # Stop + Start

# Download videos
./scripts/service/download <URL>           # Download single video
./scripts/service/download --batch        # Download from ./yt-dlp/config/urls.txt
./scripts/service/download --channels     # Download from ./yt-dlp/config/channels.txt
./scripts/service/download --help         # Show help

# Status and diagnostics
./scripts/service/status                   # Check service status, ports, disk usage, external IP
./scripts/service/check-vpn               # Verify VPN connection

# Maintenance
./scripts/service/update-images           # Pull latest container images
./scripts/lifecycle/setup-auto-update       # Setup cron job for auto-updates (Podman)
...
### yt-dlp Config Defaults (generated by `./scripts/setup/init`)
...
1. **On every start**: `./scripts/service/start` and `./scripts/service/start_no_vpn` run `./scripts/service/update-images` before starting containers
...
# 0 */3 * * * cd /path/to/project && ./scripts/service/update-images >> ./logs/update.log 2>&1
```
...
bash -n ./scripts/setup/init
bash -n ./scripts/service/start
bash -n ./scripts/service/status
bash -n ./scripts/service/download
bash -n ./scripts/service/stop

# 2. Test with Podman
./scripts/setup/init
./scripts/service/start
./scripts/service/status
./scripts/service/download --help
./scripts/service/stop

# 3. Test with Docker (if available)
CONTAINER_RUNTIME=docker ./scripts/service/start
./scripts/service/status
./scripts/service/stop
...
- **Rootless by default** — Podman runs rootless; `./scripts/service/start` uses `--userns=keep-id`
...
6. Update `./scripts/service/status` script
...
- Auto-updates via host cron (`./scripts/lifecycle/setup-auto-update`)
- `./scripts/service/start` passes `--in-pod false --podman-run-args="--userns=keep-id"` to preserve host UID ownership
