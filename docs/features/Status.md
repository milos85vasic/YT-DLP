...
- **Video-Confirmation** — per §11.4.153 every user-visible confirmed claim MUST be backed by a recorded real-use capture. The strong-model analysis path is the agent's OWN native multimodal read (Claude Opus 4.8 via the Read tool — see `docs/research/vision-path/FINDINGS.md`); local CPU models (moondream) are NOT used (too slow + hallucinate). **FIVE surfaces are so far confirmed (render/transcript scope): dashboard UI, landing 'Боба' UI, MeTube UI, postprocess status API, and the `./status` CLI.** Every OTHER row remains `PENDING — not yet recorded` until its window-scoped, `ytdlp---`-prefixed real-use capture (§11.4.154/.155) is recorded + analyzed.

> **HONEST FACT (§11.4.6 / §11.4.153):** As of this revision exactly FIVE Video-Confirmation cells are real PASSes, all analyzed by Claude Opus 4.8 native multimodal: dashboard UI render (`ytdlp---dashboard---20260615T221723Z.png`), landing 'Боба' UI render (`ytdlp---landing---20260616T074850Z.png`), MeTube UI render (`ytdlp---metube---20260616T075608Z.png`), postprocess status API render (`ytdlp---postprocess---20260616T075648Z.png`), and the `./status` CLI transcript (`ytdlp---status---20260616T075901Z.txt`). **Every one is scope=render/transcript — NOT flow-confirmed:** the end-to-end download→webready, MP3-derivation, and cookie-upload FLOWS are NOT yet video-confirmed. Every other cell honestly reads `PENDING — not yet recorded`. No row over-claims.
...
| Web-ready video transcode (`webready-<base>.mp4`, H.264/AAC, +faststart) | media_postprocessor | Pipeline feature | Implemented | `transcoder.transcode_video()` → `webready-<base>.mp4`, ffprobe-validated (`_validate_webready` asserts h264 + faststart); watcher(+min-age) + worker | pytest `transcoder`/`media_probe`; smoke | PENDING_FORENSICS (6+ real `webready-*.mp4` on disk incl. "Me at the zoo", "Rick Astley…", "Watchtower of Turkey") | PENDING — not yet recorded (transcode FLOW / artifact-playback not yet captured) |
