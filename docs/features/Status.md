...
- **Video-Confirmation** — per §11.4.153 every user-visible confirmed claim MUST be backed by a recorded real-use capture. The strong-model analysis path is the agent's OWN native multimodal read (Claude Opus 4.8 via the Read tool — see `docs/research/vision-path/FINDINGS.md`); local CPU models (moondream) are NOT used (too slow + hallucinate). **SIX surfaces are render/transcript-confirmed (dashboard UI, landing 'Боба' UI, MeTube UI, postprocess status API, `./status` CLI, `./download` CLI) AND ONE end-to-end FLOW is confirmed (download→webready transcode, via advancing live-pipeline counter + ffprobe-validated just-produced artifact).** Every OTHER row remains `PENDING — not yet recorded` until its window-scoped, `ytdlp---`-prefixed real-use capture (§11.4.154/.155) is recorded + analyzed.

> **HONEST FACT (§11.4.6 / §11.4.153):** As of this revision SEVEN Video-Confirmation cells are real PASSes across six distinct surfaces. SIX are scope=render/transcript: dashboard UI render (`ytdlp---dashboard---20260615T221723Z.png`), landing 'Боба' UI render (`ytdlp---landing---20260616T074850Z.png`), MeTube UI render (`ytdlp---metube---20260616T075608Z.png`), postprocess status API render (`ytdlp---postprocess---20260616T075648Z.png`) — the four image renders analyzed by Claude Opus 4.8 native multimodal — plus the `./status` CLI transcript (`ytdlp---status---20260616T075901Z.txt`) and the `./download` CLI render (`ytdlp---download-cli---20260616T080936Z.txt`). ONE is scope=FLOW: the download→webready transcode (`ytdlp---webready-flow---20260616T080633Z.txt`), confirmed by the live pipeline's `done` counter advancing 25→26 across two real status snapshots plus ffprobe-validation of the just-produced 1920×1080 h264 +faststart artifact (the postprocessor is a backend with no UI, so the §11.4.69/.107 sink-side evidence is the validated artifact, not a screen recording). **Still NOT flow-confirmed:** MP3-derivation and cookie-upload FLOWS (the latter operator-gated — needs fresh operator cookies; uploading test cookies would overwrite real operator state, §9.2/§11.4.122). Every other cell honestly reads `PENDING — not yet recorded`. No row over-claims.
