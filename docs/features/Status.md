...
**Revision:** 3
**Last modified:** 2026-06-16T11:35:00Z
...
- **Video-Confirmation** — per §11.4.153 every user-visible confirmed claim MUST be backed by a recorded real-use capture. The strong-model analysis path is the agent's OWN native multimodal read (Claude Opus 4.8 via the Read tool — see `docs/research/vision-path/FINDINGS.md`); local CPU models (moondream) are NOT used (too slow + hallucinate). **TWO surfaces are so far confirmed (render-only): dashboard UI + landing 'Боба' UI.** Every OTHER row remains `PENDING — not yet recorded` until its window-scoped, `ytdlp---`-prefixed real-use capture (§11.4.154/.155) is recorded + analyzed.

> **HONEST FACT (§11.4.6 / §11.4.153):** As of this revision exactly TWO Video-Confirmation cells are real PASSes — the dashboard UI render (`ytdlp---dashboard---20260615T221723Z.png`) and the landing 'Боба' UI render (`ytdlp---landing---20260616T074850Z.png`), both analyzed by Claude Opus 4.8 native multimodal, both scope=render-only (the end-to-end download + cookie-upload FLOWS are NOT yet video-confirmed). Every other cell honestly reads `PENDING — not yet recorded`. No row over-claims.
