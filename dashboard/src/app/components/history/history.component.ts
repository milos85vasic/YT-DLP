...
@Component({
  ...
  template: `
    <div class="page">
      ...
      <div class="list">
        <div class="item" *ngFor="let item of history" [class.error]="item.status === 'error'">
          <div class="icon">{{ item.status === 'error' ? '❌' : '✅' }}</div>
          <div class="info">
            ...
            <div *ngIf="item.error" class="msg">{{ item.error }}</div>
          </div>
  ...
  styles: [`
    ...
    .msg { margin-top: 4px; font-size: 12px; color: #ff5588; }
    .btn-delete {
      width: 32px;
      height: 32px;
      border-radius: 8px;
